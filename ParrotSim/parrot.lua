...
function drone:updatePolePositions()
  ...
  for i = 0, self:getPoleCount() - 1 do
    ...
    relPolePos:set(2, 0);
    local poleDistance = relPolePos:abs();
    ...
    if (projection > 0) and (math.acos(projection) < fov / 2) then
      ...
      self.relativePoleDistances[i] = poleDistance;
    else
...
--    speedVector = speedVector * (1 - math.pow(speedMixture, deltaT)) + self:getSpeed() * math.pow(speedMixture, deltaT);
    ...
    local newSpeedVector = Vector:new(speedVector[0], speedVector[1], verticalSpeed);
	tolua.takeownership(newSpeedVector); -- This is crap - needs a fix... but this fixes the memory leak
    self:setSpeed(newSpeedVector);
...
function drone:getPoleDistance(poleIndex)
  return self.relativePoleDistances[poleIndex];
end
