...
void ParrotJoystick :: calibrateAndAssociate() {
  ...
  optional1 = selectControl("optional1");
  optional2 = selectControl("optional2");
}
...
bool ParrotJoystick :: optionalButton1() const {
  if (optional1 == NULL)
    return false;
  return optional1->getPosition() > 0.5;
}

bool ParrotJoystick :: optionalButton2() const {
  if (optional2 == NULL)
    return false;
  return optional2->getPosition() > 0.5;
}
...
bool ParrotJoystick :: saveSettings(const std::string& fileName) {
  ...
  if (settingsFile.is_open()) {
    ...
    settingsFile << "Optional 1:";
    if (optional1 != NULL) {
      settingsFile << " " << optional1->getName();
    }
    settingsFile << endl;

    settingsFile << "Optional 2:";
    if (optional1 != NULL) {
      settingsFile << " " << optional2->getName();
    }
    settingsFile << endl;
            ...
            for (vector<string>::iterator it = assignmentLines.begin(); it < assignmentLines.end(); it++) {
              ...
              if (splits.size() == 2) {
                ...
                if (controlObject != NULL) {
                  ...
                  if (controlTarget == "Optional 1") {
                    optional1 = controlObject;
                    res = true;
                  }
                  if (controlTarget == "Optional 2") {
                    optional2 = controlObject;
                    res = true;
                  }
                }
              }
...
ParrotJoystick :: ParrotJoystick(int joystickID) {
  ...
  if ((SDL_JoystickNumAxes(joystick) >= 5) && (SDL_JoystickNumButtons(joystick) >= 4)) {
