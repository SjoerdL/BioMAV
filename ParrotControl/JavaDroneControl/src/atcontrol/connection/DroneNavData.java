...
public class DroneNavData
{
	...
	public static DroneNavData createFromData(byte[] buf) throws NavDataFormatException
	{
		// log.debug("Parsing navdata len=" + buf.length);
		DroneNavData data = new DroneNavData();
		data.mode = DroneNavData.Mode.BOOTSTRAP; // Assume we are in bootstrap
		...
		while (offset < buf.length)
		{
			...
			if (option_tag == NavDataTag.NAVDATA_DEMO_TAG.getValue())
			{
				...
				data.mode = DroneNavData.Mode.DEMO;
			...
			{
				// log.warn("Skipping unknown NavData option with tag=" +
				// option_tag);
			}
...
/*
	private static List<VisionTag> parseVisionTags(ARNavData data, byte[] buf, int offset) throws NavDataFormatException
	{
	...
	private static void parseDemoNavData(DroneNavData data, byte[] buf, int offset) throws NavDataFormatException
	{
	...
	private static void parseState(DroneNavData data, int state)
	{
	...
	public static void printState(DroneNavData data)
	{
