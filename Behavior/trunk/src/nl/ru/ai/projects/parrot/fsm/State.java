...
public class State {
     ...
     * @param value True when the position of the drone should be logged, false otherwise
     */
    ...
    /**
     * Sets whether or not to output the name of the state when that state becomes active
     * @param value True when to output the current state, false otherwise
     */
     ...
     * @param behavior The entry behavior
     */
     ...
     * @param behavior The exit behavior
     */
    ...
    /**
     * Sets whether or not the state is a final state (i.e. no transitions will occur)
     * @param isFinal True when the state is a final state, false otherwise (default is false)
     */
    ...
    public void reset() {
    	isActive = false;
    	writeCounter = 0;
    	for ( int i=0; i<behaviors.size(); i++ ) 		// Reset behaviors
    		behaviors.get(i).reset();
    	for ( int j=0; j<transitions.size(); j++ ) { 	// Reset transitions
    		Transition transition = transitions.get(j);
    		transition.reset();
    		if ( resetStates.add(transition.getConnectedState()) ) 	// Avoid cyclic structures
    ...
    /**
     * Runs the behaviors in a state
     * Note that each behavior needs to decide for itself whether or not to execute
